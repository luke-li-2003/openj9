...
static TR::Register *inlineStringCodingHasNegativesOrCountPositives(TR::Node *node,
   ...
   // the result is length by default if isCountPostive, otherwise it's 0
   if (isCountPositives)
      generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, returnReg, lengthReg);
   else
      generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, returnReg, 0);
   ...
   // if the result is less than zero, we should return 0 for isCountPostives, 1 otherwise
   if (isCountPositives)
      {
      generateTrg1Src3Instruction(cg, TR::InstOpCode::isellt, node, returnReg, indexReg, returnReg, cr6);
      }
   else
      {
      generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, tempReg, 1);
      generateTrg1Src3Instruction(cg, TR::InstOpCode::isellt, node, returnReg, tempReg, returnReg, cr6);
      }
   generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node, endLabel, cr6);
   // if we only have one byte end it here
   ...
   if (isCountPositives) // count the number of leading zeros in the resulting vector
      {
      ...
      {
      generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, returnReg, 1);
      }
   generateLabelInstruction(cg, TR::InstOpCode::b, node, endLabel);
   ...
   deps->stopUsingDepRegs(cg, returnReg);
