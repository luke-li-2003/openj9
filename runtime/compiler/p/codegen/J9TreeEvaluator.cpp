...
hashCodeHelper(TR::Node *node, TR::CodeGenerator *cg, TR::DataType elementType,
   ...
   TR::Register *lowReg = cg->allocateRegister(TR_VRF);
   ...
   switch (elementType)
      ...
      case TR::Int8:
         ...
         // fall through
      case TR::Int16:
         low4Reg = cg->allocateRegister(TR_VRF);
         // fall through
      case TR::Int32:
         break;
      default:
   ...
   static uint32_t multiplierVectors_be8[20] = {0x50A9DE01, 0x50A9DE01, 0x50A9DE01, 0x50A9DE01,
   ...
   static uint32_t multiplierVectors_le8[20] = {0x50A9DE01, 0x50A9DE01, 0x50A9DE01, 0x50A9DE01,
   ...
   static uint32_t multiplierVectors_be16[20] = {0x94446F01, 0x94446F01, 0x94446F01, 0x94446F01,
      0x67E12CDF, 887503681, 28629151, 923521, 29791, 961, 31, 1};
   static uint32_t multiplierVectors_le16[12] = {0x94446F01, 0x94446F01, 0x94446F01, 0x94446F01,
      1, 31, 961, 29791, 923521, 28629151, 887503681, 0x67E12CDF};
   static uint32_t multiplierVectors_be32[8] = {923521, 923521, 923521, 923521,
                                                29791, 961, 31, 1};
   static uint32_t multiplierVectors_le32[8] = {923521, 923521, 923521, 923521,
                                                1, 31, 961, 29791};
   ...
   // load the initial value in the lowest word if necessary
   if (nonZeroInitial)
      {
      generateTrg1Src1Instruction(cg, TR::InstOpCode::mtvsrd, node, low4Reg, hashReg);
   ...
   // vtmp2Reg = (valueReg & 0xF) << 3 i.e. the number of bits that are mis-aligned
   loadConstant(cg, node, 0xF, tempReg);
   generateTrg1Src2Instruction(cg, TR::InstOpCode::AND, node, tempReg, valueReg, tempReg);
   ...
   switch (elementType)
      {
      case TR::Int8:
         // unpack into halfwords, then words
         ...
         generateTrg1Src1Instruction(cg, TR::InstOpCode::vupklsh, node, high4Reg, vtmp2Reg);
         if (!isSigned)
            {
      ...
      case TR::Int16:
         ...
         break;
      case TR::Int32:
         // equivalent of a simple move since the mask contains all ones
         generateTrg1Src2Instruction(cg, TR::InstOpCode::vand, node, low4Reg, vtmp1Reg, vunpackMaskReg);
         break;
      default:
         TR_ASSERT_FATAL(false, "Unsupported hashCodeHelper elementType");
      }
   ...
   switch (elementType)
      ...
      case TR::Int8:
         if (isSigned)
            {
            generateTrg1MemInstruction(cg, TR::InstOpCode::lhax, node, tempReg, TR::MemoryReference::createWithIndexReg(cg, valueReg, constant0Reg, 2));
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, tempReg, tempReg, 8);
            }
         else
            {
   ...
   if (comp->getSymRefTab()->isNonHelper(node->getSymbolReference(), TR::SymbolReferenceTable::singlePrecisionSQRTSymbol))
      ...
      case TR::jdk_internal_util_ArraysSupport_vectorizedHashCode:
         if (!TR::Compiler->om.canGenerateArraylets()
             ...
             && comp->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX)
             && !comp->compileRelocatableCode()
             && cg->getSupportsInlineVectorizedHashCode()
             && node->getArgument(4)->getOpCode().isLoadConst()
#ifdef J9VM_OPT_JITSERVER
             && !comp->isOutOfProcessCompilation()
#endif
            )
